...
$dir = $_GET['d'];
if (($valid == 1) && ($perm['MkD'])) {
  if(@createDir($uploadDirectory . $dir . '/')) {
    header('Message: The directory was successfully created.');
    header('Status: 1');
  }
  else {
    header('Message: The directory was not successfully created: ' . addslashes($usererrors));
    header('Status: 0');
  }
}
else {
