...
static $home, $dirid, $fileid, $n, $e, $t, $f, $d;
...
if ($perm['View']) {
//$dir,$nameFilter = null,$extFilter = null,$hiddenFiles = null,$type = false,$recursive = false,$mode = false,$data = array('backup' => true,'dot' => true,'size' => true,'lastMod' => true,'ext' => true,'name' => true,'owner' => true,'mime' => false,'content' => false) {
  if ($data = listFiles($uploadDirectory . $dirPath,$n,$e,$lockedFiles,false,false,'seperate',array('lastMod' => true,'owner' => true,'size' => true,'ext' => true,'content' => 100))) {
    ...
    foreach ($data['dirs'] as $dir) {
      ...
      echo '<a href="download_dir.php?stage=2&dir=' . urlencode($dirPath . $dir['file']) . '"><img src="images/save.png" /></a></td><td><img src="images/folder.png" />&nbsp; <a href="viewdir.php?d=' . urlencode($dirPath . $dir['file']) . $cleanUrl . '">' . wordwrap($dir['file'],25,'<br />',true) . '</a></td><td>' . $dir['lastMod'][1] . '</td><td>' . $dir['owner'] . '</td><td>' . $dir['size'][1] . '</td></tr>' . "\n";
    }
    foreach ($data['files'] as $file) {
      ...
      echo '<a href="download_file.php?stage=3&file=' . urlencode($dirPath . $file['file']) . '"><img src="images/save.png" /></a></td><td><img src="images/file.png" />&nbsp;<a href="viewfile.php?file=' . urlencode($dirPath . $file['file']) . '">' . wordwrap($file['file'],25,'<br />',true) . '</a></td><td>' . $file['lastMod'][1] . '</td><td>' . $file['owner'] . '</td><td>' . $file['size'][1] . '</td><td>';
      switch($file['ext']) {
        ...
        echo '<div style="font-size: 8px; white-space: pre-line;">' . wordwrap(htmlentities($file['content']),30,'<br />',true) . '</div>';
        break;
      }
      echo '</td></tr>' . "\n";
    }
